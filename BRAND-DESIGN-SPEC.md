...
## 10. taste-skill 审查与规范（反 AI 味）

> 总则：taste-skill 是让界面"像人做的、不像 AI 批量生成"的审查标准。
> **每次 UI 改动、每个新页面、每次发布前都必须过一遍本章。** 它同时约束文案、视觉与工程三个层面。

### 10.1 负面清单（禁止出现）

| # | 禁止项 | 正确做法 |
|---|---|---|
| 1 | 步骤数字装饰（`01/02/03`、Step 1/2/3 视觉编号） | 用真实标签（如 "Windows / 首次启动 / 升级 / 校验"） |
| 2 | 破折号 `—`（em-dash） | 用逗号或分句 |
| 3 | 每行超过 1 个间隔点 `·` | 单行最多 1 个；可拆行 |
| 4 | 装饰性小圆点（列表/标题前的圆点装饰） | 用品牌色小方块/对勾/短横线代替 |
| 5 | 页脚浮动版本号（如 `v1.1.3`） | 版本号只出现在必要处（下载页/changelog/JSON-LD） |
| 6 | 非标准品牌图标（自绘简化 logo） | 用 Simple Icons 等标准免费品牌 SVG |
| 7 | 内联 `style="..."` 写样式 | 必须收敛为样式表类（如 `.section-tight`、`.hw-note`） |
| 8 | 同排控件宽度不统一 | 等宽对齐（导航胶囊、语言选择器等） |
| 9 | 同组控件字体/字号/字重不一致 | 统一字重（如导航按钮 600）、字号、字体栈 |
| 10 | 未批准的第二强调色 | 单一强调色；物理世界等特殊场景需用户批准 |
| 11 | AI 营销空话、形容词堆砌 | 直白描述能力与价值 |
| 12 | 序数混用数字与汉字（"第1位"） | 统一汉字序数（"第一位"） |
| 13 | 事实性错误文案（托管方/下载源等） | 随技术栈变更同步更新文案（如 Vercel→Cloudflare Pages） |
| 14 | 无意义装饰动画/装饰图形 | 动效必须服务信息层级 |

### 10.2 视觉纪律（正面要求）

1. **单一强调色 + AA 对比**：一次只用一个品牌强调色；正文 4.5:1、大字 3:1。
2. **清晰视觉层级**：主 CTA 大而实（品牌色实底），次级入口小而弱（描边/弱化色）；禁止同级控件互相抢视觉。
3. **对齐与对称**：同排控件等高、等宽、等字重、等内边距；网格栅格对齐。
4. **克制动效**：0.16s（hover）～0.7s（入场）；`prefers-reduced-motion` 必须降级为静态。
5. **语义色只做语义**：绿=就绪/成功、琥珀=即将推出、灰=评估/次要、蓝=info、红=危险/错误；不与品牌强调色混用。
6. **文字节奏**：按钮动词开头（下载/登录/订阅/开始使用）；短句；无破折号；信息密度适中。

### 10.3 工程纪律（代码层面）

1. 样式一律进 `style.css` 的具名类/工具类，禁止内联样式。
2. 每次改 `main.js`/`zh.js`/`style.css` 必须 bump 页面引用版本号（`?v=N` → `?v=N+1`），避免旧缓存。
3. i18n 一致性：所有可见文本挂 `data-i18n`，新增键四语同步；禁止页面残留未翻译中文。
4. 可访问性：语义化标签（`button`/`a`/`label`）、`aria-hidden` 装饰、`:focus-visible` 轮廓、对比度达标。
5. 响应式：`960px`/`560px` 断点检查；移动端不出现横向滚动与裁切。
6. 事实准确：托管/分发/联系方式等随技术栈变化同步更新（曾出现 "托管于 Vercel" 未随迁移更新的事件）。
...
- [ ] 四语切换无漏译、无残留中文硬编码
- [ ] 移动端（≤960px / ≤560px）正常
- [ ] `prefers-reduced-motion` 下无卡顿动画
- [ ] `node tests/run-tests.js` 全绿（含占位符阻断）
- [ ] 页面加入测试清单（`HTML_FILES` + `REQUIRED_IDS`）
- [ ] 版本号已 bump、已部署、已线上抽查

### 10.5 典型修正示例（本品牌实际发生过）

| 问题 | 修正 |
|---|---|
| 页脚显示 `v1.1.3` | 移除版本号 |
| 安装提示 `01 安装 → 02 配置` | 改为真实标签 "Windows / 首次启动 / 升级 / 校验" |
| 首页 "第1位AI数字员工" | 统一为 "第一位AI数字员工" |
| 页脚 "托管于 Vercel"（实际已迁 Cloudflare） | 同步事实文案 |
| 自绘简化 Android/Apple/Linux 图标 | 替换为 Simple Icons 标准品牌 SVG |
| HTML 内联 `style="padding-top:56px"` 等 9 处 | 收敛为 `.section-tight`/`.hw-note` 等类 |
| 导航语言选择器字重 600 vs 按钮 400 | 统一 600 |
| 导航 4 按钮宽度参差 | JS 取当前语言最大宽度统一 |
| 首页 4 平台下载按钮排布失衡 | 主按钮 + 弱化小胶囊两级层级 |
...
- [ ] **过 §10 taste-skill 审查**：负面清单逐条 + 视觉纪律 + 工程纪律（禁止内联样式/版本号 bump/i18n 无漏译/等宽对齐/AA 对比）
