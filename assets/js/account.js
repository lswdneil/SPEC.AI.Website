...
(function () {
  ...
  function render(user, stats, recent) {
  ...
  function init() {
    ...
    Promise.all([apiGet('/api/me'), apiGet('/api/stats')]).then(function (res) {
      var me = res[0], st = res[1];
      ...
      loading.hidden = true;
      body.hidden = false;
      render(me.data.user, statsData, recent);
    }).catch(function () {
