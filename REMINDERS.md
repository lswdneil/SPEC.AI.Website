...
- **Git 推送规则（2026-08-24 用户明确；2026-08-25 强化）**：日常开发提交仅在本地（`git commit`），**不 push 到 GitHub**；**任何一次远程推送都必须先获得用户明确授权**——即使前一次改动已获授权推送，也不得默认后续改动可连续推送；每次独立改动均须单独请求授权。注意：Cloudflare 是 Git 集成部署（push 即上线），因此**不推送 = 不上线**，线上更新需用户授权 push。⚠️ 已发生两次违规（缓存修复 e9eb0a2、页脚版权 21f5190 未经授权即推送），以此为戒，推送前必须停下等授权。
